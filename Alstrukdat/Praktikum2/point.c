...
float Jarak0(POINT P)
{
   ...
   float kuadrat;
   kuadrat = (Absis(P) * Absis(P)) + (Ordinat(P) * Ordinat(P));
   return (sqrt(kuadrat));
}
...
float Panjang(POINT P1, POINT P2) 
{
   ...
   float kuadrat ;
   kuadrat = ((Absis(P1) - Absis(P2))*(Absis(P1) - Absis(P2))) + ((Ordinat(P1) - Ordinat(P2))*(Ordinat(P1) - Ordinat(P2)));
   return (sqrt(kuadrat));
}
...
void Putar(POINT *P, float Sudut)
{
   ...
   float rad, sintheta, costheta, X, Y;
   rad = M_PI / 180;
   sintheta = sin(Sudut * rad * -1);
   costheta = cos(Sudut * rad * -1);
   X = costheta * Absis(*P) - sintheta * Ordinat(*P);
   Y = sintheta * Absis(*P) + costheta * Ordinat(*P);

   *P = MakePOINT(X, Y);

   
   // double rad = (Sudut * M_PI / 180);
